...
def input_probabilistic_transition_system(filename=None, use_file=True):
    ...
    if use_file and filename:
        ...
        terminating_vector = list(
            map(int, lines[num_states + 1 : num_states + 1 + num_states])
        )
        ...
        raise NotImplementedError(
            "Command-line input is not supported in the Streamlit app."
        )
...
def refine_relation(R, transition_matrix, terminating_vector):
    ...
    while worklist:
        ...
        for block_id, mass_groups in block_to_states.items():
            if (
                len(mass_groups) > 1
            ):  # Only split if there are different probability masses
                ...
                for mass, states in mass_groups.items():
                    if len(states) < len(
                        original_block
                    ):  # Compare against original block size
...
def compute_equivalence_classes(R, num_states, terminating_vector):
    ...
    for i in range(num_states):
        ...
        class_termination_status[class_id] = any(
            terminating_vector[s] == 1 for s in comp
        )
...
def compute_minimized_transition_matrix(
    transition_matrix, equivalence_classes, transition_labels
):
    """
    ...
    state_class_map = {
        s: c for c, states in equivalence_classes.items() for s in states
    }
    ...
    minimized_T = np.divide(
        minimized_T, row_sums[:, np.newaxis], where=row_sums[:, np.newaxis] != 0
    )
...
def analyze_state_differences(
    idx1, idx2, T, Term, D_classes, equivalence_classes, minimized_T, class_termination
):
    """
    ...
    if Term[idx1] == Term[idx2]:
        ...
        if len(c) != A_eq.shape[1]:
            raise ValueError(
                f"Dimension mismatch: c length ({len(c)}) != A_eq columns ({A_eq.shape[1]})"
            )
        ...
        if res.success:
            ...
            for i in range(num_classes):
                for j in range(num_classes):
                    ...
                    if (
                        flow > 1e-8 and contrib > 1e-8
                    ):  # Only keep nonzero contributions
                        moves.append(
                            (
                                i,
                                j,
                                minimized_T[class1, i],
                                minimized_T[class2, j],
                                contrib,
                            )
                        )
...
def bisimulation_distance_matrix(T, Term, tol=1e-6, max_iter=100):
    ...
    equivalence_classes, _, class_termination = compute_equivalence_classes(
        R_matrix, n, Term
    )
            ...
            if T[i, j] > 0:
                ...
                if label:
                    ...
                    dot.edge(
                        f"{prefix} {i+1}",
                        f"{prefix} {j+1}",
                        label=f"{label_text} ({T[i, j]:.2f})",
                    )
                else:
                    dot.edge(
                        f"{prefix} {i+1}", f"{prefix} {j+1}", label=f"{T[i, j]:.2f}"
                    )
    return dot.source
