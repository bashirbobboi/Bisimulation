...
def test_bisim_and_file_outputs(tmp_path, monkeypatch):
    ...
    internal.write_text(
        json.dumps({"T": [[0.5, 0.5], [0.0, 1.0]], "Term": [0, 1], "labels": {}})
    )
...
def test_dist_and_heatmap(tmp_path, monkeypatch):
    ...
    internal = tmp_path / "int.json"
    internal.write_text(
        json.dumps({"T": [[1.0, 0.0], [0.0, 1.0]], "Term": [0, 0], "labels": {}})
    )
...
def test_explain_and_class_commands(tmp_path, monkeypatch):
    ...
    internal = tmp_path / "int.json"
    internal.write_text(
        json.dumps({"T": [[1.0, 0.0], [0.5, 0.5]], "Term": [0, 0], "labels": {}})
    )
...
def test_simulate_and_compare(tmp_path, monkeypatch):
    ...
    internal = tmp_path / "int.json"
    internal.write_text(
        json.dumps({"T": [[0.0, 1.0], [1.0, 0.0]], "Term": [0, 1], "labels": {}})
    )

    # Monkey-patch np.random.choice to always return state 0
    monkeypatch.setattr(np.random, "choice", lambda a, p=None, *args, **kwargs: 0)
