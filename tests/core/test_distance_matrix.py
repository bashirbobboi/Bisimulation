"""
Unit tests for bisimulation_distance_matrix and equivalence class grouping by distance.
Covers simple hand-computed examples, distance properties, and class grouping logic.
"""
...
def test_simple_distance(T, Term, expected):
    ...
    D, *_ = bisimulation_distance_matrix(T, Term, max_iter=10)
    assert np.allclose(D, expected, atol=1e-6)


def test_distance_properties():
    """Test general properties of the distance matrix (symmetry, triangle inequality, etc)."""
    # Create a random PTS
    n = 5
    T = np.random.dirichlet(np.ones(n), size=n)
    Term = np.random.choice([0, 1], size=n)

    D, *_ = bisimulation_distance_matrix(T, Term)
